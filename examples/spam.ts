...
async function main() {
  ...
  const session = await client.createSession({ type: "user_verification", modules: ["spam", "bot"] });
  const result = await client.spam(session).evaluate("Buy now!!! Limited offer click here");
  console.log(result);
}
